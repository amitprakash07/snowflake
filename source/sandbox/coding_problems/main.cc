...
vector<vector<int>> findWinners(vector<vector<int>>& matches)
{
    ...
    for (std::set<int>::const_iterator it = winners.cbegin(); it != winners.cend(); ++it)
    {
    ...
    for (std::map<int, int>::const_iterator it = loosers.cbegin(); it != loosers.cend(); ++it)
    {
        if (it->second == 1)
        {
...
std::map<int, int> PascalsTriangle::factorials;
std::unordered_map<PascalsTriangle::Combinatorics, int, PascalsTriangle::CombinatoricsHash> PascalsTriangle::coefficients_map;

int main(int argCount, char* argc[])
{
    INSTANTIATE_PROBLEM(PascalsTriangle);

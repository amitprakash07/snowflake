...
engine::SharedPointer<engine::Windows::WindowingSystem> engine::Windows::WindowingSystem::getWindowingSystem(bool fullScreen, int width, int height)
{
    if (mWindowingSystem.isNull())
    {
        SharedPointer<Windows::WindowingSystem> temp(new WindowingSystem(), "Windows::WindowingSystem");
...
HWND engine::Windows::WindowingSystem::CreateMainWindowHandle(const HINSTANCE i_thisInstanceOfTheProgram, const int i_initialWindowDisplayState) const
{
    ...
    HWND mainWindow;
    {
        ...
        if (Networking::NetworkManager::isServerInstance())
        ...
        mainWindow = CreateWindowEx(windowStyle_extended,
                                    s_mainWindowClassName,
                                    windowCaptionString.c_str(),
                                    windowStyle,
                                    position_x,
                                    position_y,
                                    width,
    ...
    // Change the window's size based on the desired client area resolution
    {
        ...
        // Resize the window
        {
            ...
            if (MoveWindow(mainWindow,
                           windowCoordinates.left,
                           windowCoordinates.top,
...
LRESULT CALLBACK engine::Windows::WindowingSystem::OnMessageReceived(HWND i_window, UINT i_message, WPARAM i_wParam, LPARAM i_lParam)
{
    ...
    case WM_CHAR:
    {
        ...
        engine::SharedPointer<engine::Windows::WindowingSystem> tempWindowingSystem = engine::EngineCore::getWindowingSystem();
        engine::EngineCore::getMessagingSystem()->sendMessage(temp, tempWindowingSystem.CastSharedPointer<engine::RTTI>(), reinterpret_cast<void*>(i_wParam));
        break;
    }
    case WM_KEYDOWN:
    {
    ...
    case WM_LBUTTONDOWN:
    {
        ...
        engine::SharedPointer<engine::MouseController> tempMouseController = engine::EngineCore::getMouseInputController();
...
bool engine::Windows::WindowingSystem::UnregisterMainWindowClass(const HINSTANCE i_thisInstanceOfTheProgram) const
{
...
bool engine::Windows::WindowingSystem::IsSameType(engine::SharedPointer<RTTI>, std::string) const
{
    return true;
}

std::string engine::Windows::WindowingSystem::GetType() const
{
    return "";
}

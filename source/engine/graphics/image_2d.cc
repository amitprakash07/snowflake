...
void amit::image::WriteColorBufferToPPM(const engine::graphics::ColorBuffer& color_buffer, const char* filename)
{
    FILE* file = fopen(filename, "wb");
    fprintf(file, "P6\n%d %d\n255\n", color_buffer.GetWidth(), color_buffer.GetHeight());
    fwrite(color_buffer.GetData(),
           sizeof(engine::graphics::Rgb8),
           static_cast<size_t>(color_buffer.GetWidth()) * color_buffer.GetHeight(),
           file);
    fclose(file);
}

void amit::image::WriteDepthBufferToPPM(const engine::graphics::DepthBuffer& depth_buffer, const char* filename)
{
    FILE* file = fopen(filename, "wb");

...
Math::Vector3 Physics::Octree::GetMax() const { return max; }

Math::Vector3 Physics::Octree::GetMin() const { return min; }

uint32_t Physics::Octree::GetNodeCount() const { return mCountOfNode; }

Physics::OctreeNode* Physics::Octree::GetNodes() const {
  return mNodes;
}

uint32_t Physics::Octree::Level() const { return mLevel; }

uint32_t Physics::Octree::GetCountOfNodesInLevel(
    uint32_t levelIndex) const {
  return static_cast<uint32_t>(std::pow(8.0f, levelIndex));
}

Physics::OctreeNode* Physics::Octree::GetNodesInLevel(
    uint32_t levelIndex) const {
...
void Physics::Octree::InitFromRange(uint32_t level,
                                            Math::Vector3 min,
                                            Math::Vector3 max) {
  mLevel = level;
...
void Physics::Octree::InitFromFile(const char* pFile) {
  ...
  infile.read(pBuffer, size);
  {
    ...
    offset += sizeof(uint32_t);
    memcpy(reinterpret_cast<uint8_t*>(pBuffer) + offset,
           reinterpret_cast<uint8_t*>(&min), sizeof(Math::Vector3));
    offset += sizeof(Math::Vector3);
    memcpy(reinterpret_cast<uint8_t*>(pBuffer) + offset,
           reinterpret_cast<uint8_t*>(&max), sizeof(Math::Vector3));
    offset += sizeof(Math::Vector3);

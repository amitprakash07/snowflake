...
Math::Plane::Plane(Vector3 iNormal, Vector3 iPointOnPlane) {
  normal = iNormal;
  float a = iNormal.x;
  float b = iNormal.y;
  float c = iNormal.z;
...
Math::Vector3 Math::Plane::ClosestPointFromPointOnPlane(
    Vector3 iPointOnPlane) const {
  float t = (normal.dot(iPointOnPlane) - distance) / normal.dot(normal);
  return (iPointOnPlane - (normal * t));
}

Math::Vector3 Math::Plane::getPlaneNormal() const {
  return normal;
}

float Math::Plane::getDistanceFromOrigin() const { return distance; }

Math::Plane Math::Plane::ComputePlane(Vector3 iA, Vector3 iB,
                                                      Vector3 iC) {
  Plane p;
...
bool Math::Sphere::IntersectSphere(Sphere iOther) const {
...
void Math::ClosestPointFromPointOnLineSegment(Vector3 c, Vector3 a,
                                                      Vector3 b, float &t,
                                                      Vector3 &d) {
  Vector3 ab = b - a;

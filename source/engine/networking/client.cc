...
Networking::Client::Client(int iMaxPlayer) {
  mNumSockets = 1;
  mNumIPs = 1;
...
void Networking::Client::ShutDownClient() {
  client->Shutdown(300);
  RakNet::RakPeerInterface::DestroyInstance(client);
}

void Networking::Client::CreateClientInterface() {
...
void Networking::Client::ClientStartup() {
  mSocketDescriptor = RakNet::SocketDescriptor(atoi(clientPort.c_str()), 0);
  mSocketDescriptor.socketFamily = AF_INET;
  client->Startup(8, &mSocketDescriptor, 1);
  client->SetOccasionalPing(true);

  RakNet::ConnectionAttemptResult car =
      client->Connect(serverIP.c_str(), atoi(serverPort.c_str()), "admin",
                      static_cast<int>(strlen("admin")));
  RakAssert(car == RakNet::CONNECTION_ATTEMPT_STARTED);
}

void Networking::Client::ReceivePackets() {
  for (mPacket = client->Receive(); mPacket;
       client->DeallocatePacket(mPacket), mPacket = client->Receive()) {
    ...
    switch (mPacketIdentifier) {
      ...
      case ID_LOAD_CURRENT_PLAYERS: {
        ...
        std::vector<SharedPointer<NetworkPlayer>> controlNetworkPlayer =
            GetControlPlayer();
        for (std::vector<SharedPointer<NetworkPlayer>>::iterator i =
                 controlNetworkPlayer.begin();
...
void Networking::Client::SetIP() {
  serverIP = "127.0.0.1";
  clientPort = "6000";
}

SharedPointer<Networking::NetworkPlayer>
Networking::Client::InstantiateNetworkPlayer(
...
void Networking::Client::addToNetworkPlayerList(
...
std::vector<SharedPointer<Networking::NetworkPlayer>>
Networking::Client::GetControlPlayer() {
  std::vector<SharedPointer<Networking::NetworkPlayer>>
...
SharedPointer<Networking::NetworkPlayer>
Networking::Client::GetNetworkPlayer(RakNet::NetworkID i_networkID) {
  for (std::map<RakNet::NetworkID,
                SharedPointer<Networking::NetworkPlayer>>::iterator i =
           mPlayerLists.begin();
       i != mPlayerLists.end(); ++i) {

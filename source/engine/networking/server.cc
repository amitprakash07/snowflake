...
Networking::Server::Server(int iNumPlayers) {
  mNumSockets = 1;
  mNumIPs = 1;
...
void Networking::Server::CreateServerInterface() {
  mServer = RakNet::RakPeerInterface::GetInstance();
  WindowsUtil::Assert(mServer != nullptr,
                      "Unable to create the server interface");
  mServer->SetIncomingPassword("admin", static_cast<int>(strlen("admin")));
  mServer->SetTimeoutTime(30000, RakNet::UNASSIGNED_SYSTEM_ADDRESS);
}

void Networking::Server::ReceivePackets() {
  for (mPacket = mServer->Receive(); mPacket;
       mServer->DeallocatePacket(mPacket), mPacket = mServer->Receive()) {
    mPacketIdentifier = GetPacketIdentifier(mPacket);
    RakNet::BitStream receiveBitStream(mPacket->data, mPacket->length, false);
    switch (mPacketIdentifier) {
      case ID_DISCONNECTION_NOTIFICATION:
        ...
        std::cout << "Network Disconnected  "
                  << mPacket->systemAddress.ToString(true) << std::endl;
        ;
        break;
      ...
      case ID_NEW_INCOMING_CONNECTION: {
        ...
        for (std::map<RakNet::NetworkID, SharedPointer<NetworkPlayer>>::iterator
          ...
          Graphics::RGBAColor iColor = tempMeshObject->GetVertexColor();
      ...
      case ID_INCOMPATIBLE_PROTOCOL_VERSION:
        WindowsUtil::displayToOutPutWindow(
            "Incompatible Connection Protocol\n");
      ...
      case ID_SPAWN_PLAYER: {
        ...
        SharedPointer<NetworkPlayer> tempNetworkPlayer =
            NetworkPlayer::CreateNetworkPlayer(tempMeshName, tempMaterialName,
                                               tempTransform, tempColor);
...
void Networking::Server::ServerStartUp() {
  mSocketDescriptor.port =
      atoi(mServerPort.c_str());             // Set the port for the server
  mSocketDescriptor.socketFamily = AF_INET;  // IPv4 Protocol
  bool serverStartUpStatus = mServer->Startup(mMaxPlayers, &mSocketDescriptor,
                                              1) == RakNet::RAKNET_STARTED;
  WindowsUtil::Assert(serverStartUpStatus, "Unable to start the server");
  mServer->SetMaximumIncomingConnections(mMaxPlayers);
  mServer->SetOccasionalPing(
      true);  // Keep pinging still if nothing is send or receive
  mServer->SetUnreliableTimeout(1000);
  DebugServerInfo();
}

void Networking::Server::ShutDownServer() {
  mServer->Shutdown(300);
  RakNet::RakPeerInterface::DestroyInstance(mServer);
}

void Networking::Server::DebugServerInfo() {
  ...
  for (unsigned int i = 0; i < sockets.Size(); i++) {
    WindowsUtil::displayToOutPutWindow(
        sockets[i]->GetBoundAddress().ToString(true));
    mNumSockets++;
  }
  ...
  WindowsUtil::displayToOutPutWindow(
      mServer->GetGuidFromSystemAddress(RakNet::UNASSIGNED_SYSTEM_ADDRESS)
          .ToString());
}

SharedPointer<Networking::NetworkPlayer>
Networking::Server::InstantiateNetworkPlayer(
    RakNet::NetworkID iNetworkID, std::string i_meshFileName,
    std::string i_materialName, Math::Transform clientPlayerTransform,
    Graphics::RGBAColor iColor) {
  SharedPointer<NetworkPlayer> newPlayer = NetworkPlayer::CreateNetworkPlayer(
      i_meshFileName, i_materialName, clientPlayerTransform, iColor);
...
void Networking::Server::addToNetworkPlayerList(
    SharedPointer<MeshObject> IObject, bool i_isControlPlayer) {
  SharedPointer<NetworkPlayer> tempNetworkPlayer =
      NetworkPlayer::CreateNetworkPlayer(IObject);
  tempNetworkPlayer->SetNetworkIDManager(&mNetworkIDManager);
  mPlayerLists[tempNetworkPlayer->GetNetworkID()] = tempNetworkPlayer;
  tempNetworkPlayer->SetControlPlayer(i_isControlPlayer);
}

SharedPointer<Networking::NetworkPlayer>
Networking::Server::GetControlPlayer() {
  for (std::map<RakNet::NetworkID,
                SharedPointer<Networking::NetworkPlayer>>::iterator i =
           mPlayerLists.begin();
       i != mPlayerLists.end(); ++i) {
    if (i->second->GetControlPlayerStatus()) return i->second;
  }
  return SharedPointer<Networking::NetworkPlayer>();
}

SharedPointer<Networking::NetworkPlayer>
Networking::Server::GetNetworkPlayer(RakNet::NetworkID i_networkID) {
  for (std::map<RakNet::NetworkID,
                SharedPointer<Networking::NetworkPlayer>>::iterator i =
           mPlayerLists.begin();
       i != mPlayerLists.end(); ++i) {
    if (i->first == i_networkID) return i->second;
  }
  return SharedPointer<Networking::NetworkPlayer>();
}

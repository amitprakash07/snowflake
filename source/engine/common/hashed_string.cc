...
StringHash::StringHash() { StringHash("\0"); }  // end default constructor

StringHash::StringHash(const char* i_string) : mHash(Hash(i_string)) {
#ifdef _DEBUG
  mString = _strdup(i_string);
#endif
}  // end parameterized constructor

StringHash::StringHash(const StringHash& i_other) : mHash(i_other.mHash) {
#ifdef _DEBUG
...
unsigned __int32 StringHash::getHash() const { return mHash; }  // end getHash()

unsigned __int32 StringHash::Hash(const char* i_string) {
  return (Hash(reinterpret_cast<void*>(const_cast<char*>(i_string)),
               strlen(i_string)));
}  // end Hash(char*)

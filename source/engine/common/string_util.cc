...
std::string util::RemoveString(const std::string& remove_from,
                               const std::string& remove_with, bool beginning) {
  std::string returnString;
  if (beginning) {
    char* returnStringInChar =
        new char[remove_from.size() - remove_with.size() + 1];
    memcpy(returnStringInChar, remove_from.c_str() + remove_with.size(),
           remove_from.size() - remove_with.size());
  ...
  for (char ch : path) {
    if (ch == '/') {
      if (!canonical_path.empty() && canonical_path.back() == '/') {
        continue;
      } else if (canonical_path.size() > 2 && canonical_path[it - 1] == '.' &&
                 canonical_path[it - 2] == '.') {

...
template <class DataType>
uint32_t engine::BinaryTree<DataType>::FindDepth(
    BinaryTreeNode<DataType>* node_to_found) {
  std::function<uint32_t(BinaryTreeNode<DataType>*, BinaryTreeNode<DataType>*,
                         bool)>
      find_depth_helper = [&](BinaryTreeNode<DataType>* node,
                              BinaryTreeNode<DataType>* find,
                              bool& is_found) -> uint32_t {
    if (node == nullptr) {
      return 0;
    }
...
uint32_t engine::BinaryTree<DataType>::FindDepth(DataType node_to_found) {
  std::function<uint32_t(BinaryTreeNode<DataType>*, DataType, bool&)>
      find_depth_helper = [&](BinaryTreeNode<DataType>* node, DataType find,
                              bool& is_found) -> uint32_t {
    if (node == nullptr) {
      return 0;
    }
...
void engine::BinaryTree<DataType>::UnitTest() {
  {
    ...
    int_tree.Construct(
        {0, 1, 2, 3, 4, INT_MAX, 5, INT_MAX, INT_MAX, INT_MAX, 6});
    {
...
DoublyLinkedList<DataType> engine::BinarySearchTree<DataType>::Convert() {
  ...
  BinaryTree<DataType>::Traverse(BinaryTree<DataType>::root,
                                 TreeTraverseOrder::InOrder, out);
...
template <class DataType>
void engine::BinarySearchTree<DataType>::Add(BinaryTreeNode<DataType>* node,
                                             DataType data) {

...
template <typename T>
inline SharedPointer<T>& SharedPointer<T>::operator=(
    SharedPointer<T>& i_other) {
  if (!isEqual(i_other)) {
...
inline bool SharedPointer<T>::isEqual(const SharedPointer<T>& i_other) const {
  if (m_WrappingObject == i_other.m_WrappingObject) return true;
  return false;
}
...
inline T* SharedPointer<T>::operator->() const {
  assert(m_WrappingObject);
  if (!this->isNull()) return m_WrappingObject;
  return nullptr;
}
...
inline bool SharedPointer<T>::deleteObject() {
  ...
  if (!this->isNull()) {
    if ((*m_referenceCount - 1) == 0) {
      ...
      if (m_WrappingObject) delete m_WrappingObject;
      if (m_referenceCount) delete m_referenceCount;
      m_WrappingObject = nullptr;
    } else
      (*m_referenceCount)--;
...
/*template<typename T>
inline SharedPointer<RTTI> SharedPointer<T>::getRawPointer() const
{

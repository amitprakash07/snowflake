...
Engine::Windows::WindowingSystem::getWindowingSystem(bool fullScreen, int width,
  ...
  if (mWindowingSystem.isNull()) {
    SharedPointer<Engine::Windows::WindowingSystem> temp(
        new WindowingSystem(), "Engine::Windows::WindowingSystem");
...
bool Engine::Windows::WindowingSystem::CreateMainWindow(
  ...
  if (mainWindowClass != NULL) {
    s_mainWindow = CreateMainWindowHandle(i_thisInstanceOfTheProgram,
                                          i_initialWindowDisplayState);
    if (s_mainWindow == nullptr) return false;
    return true;
  }
  return false;
}
...
ATOM Engine::Windows::WindowingSystem::RegisterMainWindowClass(
  ...
  wndClassEx.hIcon =
      LoadIcon(i_thisInstanceOfTheProgram, MAKEINTRESOURCE(IDI_BIG));
  wndClassEx.hIconSm =
      LoadIcon(i_thisInstanceOfTheProgram, MAKEINTRESOURCE(IDI_SMALL));
  // The cursor that should display when the mouse pointer is over windows of
  // this class
  ...
  wndClassEx.hbrBackground =
      reinterpret_cast<HBRUSH>(IntToPtr(COLOR_BACKGROUND + 1));
  // A menu can be specified that all windows of this class would use by
  // default, but usually this is set for each window individually
  ...
  if (mainWindowClass == NULL) {
    ...
    std::string errorMessage(
        "Windows failed to register the main window's class: ");
    errorMessage += WindowsUtil::GetLastWindowsError();
    MessageBox(nullptr, errorMessage.c_str(), errorCaption,
               MB_OK | MB_ICONERROR);
  }
...
HWND Engine::Windows::WindowingSystem::CreateMainWindowHandle(
  ...
  HWND mainWindow;
  {
    ...
    const DWORD windowStyle =
        ...
        | WS_CAPTION
        // The window should never change dimensions, so only a minimize box is
        // allowed
        | WS_MINIMIZEBOX
    ...
    const DWORD windowStyle_extended =
        // The following is a macro to make the extended style the default
        // top-level look
        WS_EX_OVERLAPPEDWINDOW;
    // The width and height of the window.
    // A game cares about the width and height of the actual "client area",
    // which is the part of the window that doesn't include the borders and
    // title bar; this means that if we say that a game runs at a resolution of
    // 800 x 600, the actual window will be slightly bigger than that.
    // Initially, then, the window will be created with default values that
    // Windows chooses and then resized after creation)
    ...
    // Handle to the instance of the program that this window should be
    // associated with
    const HINSTANCE hProgram = i_thisInstanceOfTheProgram;
    // Any arbitrary pointer can be associated with this window;
    // usually a program's own custom representation of the window will be used,
    // like this: void* userData = someCustomStructOrClass; Since this example
    // program is simple, though, no data will be associated:
    ...
    mainWindow = CreateWindowEx(windowStyle_extended, s_mainWindowClassName,
                                windowCaptionString.c_str(), windowStyle,
                                position_x, position_y, width, height, hParent,
                                hMenu, hProgram, userData);
  ...
  // Change the window's size based on the desired client area resolution
  {
    ...
    } nonClientAreaSize;
    {
      ...
      if (GetWindowRect(mainWindow, &windowCoordinates) == FALSE) {
        std::string errorMessage(
            "Windows failed to get the coordinates of the main window: ");
        errorMessage += WindowsUtil::GetLastWindowsError();
        MessageBox(nullptr, errorMessage.c_str(), nullptr,
                   MB_OK | MB_ICONERROR);
        return nullptr;
      }
      ...
      nonClientAreaSize.width =
          (windowCoordinates.right - windowCoordinates.left) -
          clientDimensions.right;
      nonClientAreaSize.height =
          (windowCoordinates.bottom - windowCoordinates.top) -
          clientDimensions.bottom;
    }
    // Resize the window
    {
      BOOL shouldTheResizedWindowBeRedrawn = TRUE;
      if (MoveWindow(mainWindow, windowCoordinates.left, windowCoordinates.top,
                     desiredWidth + nonClientAreaSize.width,
                     desiredHeight + nonClientAreaSize.height,
...
LRESULT CALLBACK Engine::Windows::WindowingSystem::OnMessageReceived(
  ...
  switch (i_message) {
    ...
    case WM_LBUTTONDOWN: {
      ...
      SharedPointer<Engine::MouseController> tempMouseController =
...
bool Engine::Windows::WindowingSystem::UnregisterMainWindowClass(
  ...
  if (UnregisterClass(s_mainWindowClassName, i_thisInstanceOfTheProgram) !=
    ...
    std::string errorMessage(
        "Windows failed to unregister the main window's class: ");
    errorMessage += WindowsUtil::GetLastWindowsError();
    MessageBox(nullptr, errorMessage.c_str(), errorCaption,
               MB_OK | MB_ICONERROR);
    return false;
...
bool Engine::Windows::WindowingSystem::OnMainWindowClosed(
  ...
  if (!EngineCore::getWindowingSystem()->UnregisterMainWindowClass(
          i_thisInstanceOfTheProgram))
    wereThereErrors = true;
...
bool Engine::Windows::WindowingSystem::isBothSameType(SharedPointer<RTTI>,
                                                      std::string) const {
  return true;
}

std::string Engine::Windows::WindowingSystem::getTypeInfo() const { return ""; }

...
void Engine::EngineCore::Initialize(HINSTANCE hInstance, int windowLayout,
  ...
  if (!getWindowingSystem().isNull()) {
    ...
    if (!Engine::Graphics::GraphicsSystem::Initialize(
...
std::string Engine::EngineCore::getMeshFolderPath() { return meshFolderName; }
...
Engine::SharedPointer<Engine::MessagingSystem>
Engine::EngineCore::getMessagingSystem() {
  if (mMessagingSystem.isNull()) {
    SharedPointer<Engine::MessagingSystem> tempMessagingSystem =
...
Engine::SharedPointer<Engine::StringPool> Engine::EngineCore::getStringPool() {
  if (mStringPool.isNull()) {
    SharedPointer<Engine::StringPool> tempStringPool =

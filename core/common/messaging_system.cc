...
Engine::SharedPointer<Engine::MessagingSystem>
    Engine::MessagingSystem::mMessagingSystem;
...
Engine::MessagingSystem::getMessagingSystem() {
  if (mMessagingSystem.isNull()) {
    ...
    SharedPointer<Engine::MessagingSystem> temp(SharedPointer<MessagingSystem>(
        new MessagingSystem(), "Engine::MessagingSystem"));
...
void Engine::MessagingSystem::addMessageHandler(
    Engine::utils::StringHash& i_message,
    SharedPointer<IMessageHandler> i_pMessageHandler,
    Engine::typedefs::Priority i_priority) {
...
void Engine::MessagingSystem::sendMessage(Engine::utils::StringHash& i_message,
  ...
  for (size_t i = 0; i < handleCount; i++) {
    tempList[i].mHandler->HandleMessage(i_message, i_messageSender,
                                        message_data);
  }
}

bool Engine::MessagingSystem::removeMessageHandler(
    Engine::utils::StringHash& i_message,
    SharedPointer<IMessageHandler> i_messageHandler) {

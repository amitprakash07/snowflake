...
std::string Engine::Utils::RemoveString(std::string removeFrom,
                                        std::string removeWith,
                                        bool beggining) {
  std::string returnString;
  if (beggining) {
    char* returnStringInChar =
        new char[removeFrom.size() - removeWith.size() + 1];
    memcpy(returnStringInChar, removeFrom.c_str() + removeWith.size(),

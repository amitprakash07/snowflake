...
inline bool TestAABBPlane(AABBV1 b, Plane p) {
  ...
  float r = e[0] * std::fabsf(p._normal[0]) + e[1] * std::fabsf(p._normal[1]) +
            e[2] * std::fabsf(p._normal[2]);
  // Compute distance of box center from plane
  float s = p._normal.dot(c) - p._d;  // Intersection occurs when distance s
                                      // falls within [-r,+r] interval
  return std::fabsf(s) <= r;
}

inline bool TestTriangleAABB(Engine::Math::Vector3 v0, Engine::Math::Vector3 v1,
                             Engine::Math::Vector3 v2, AABBV1 b) {
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      ...
      // If the projection intervals [?r, r] and min(p0, p1, p2), max(p0, p1,
      // p2) are disjoint for the given axis, the axis is a separating axis and
      // the triangle and the AABB do not overlap.
      if (max < -r || min > r) return false;  // Axis is a separating axis
    }
  }
  // Test the three axes corresponding to the face normals of AABB b (category
  // 1). Exit if...
  // ... [-e0, e0] and [min(v0.x,v1.x,v2.x), max(v0.x,v1.x,v2.x)] do not overlap
  if (std::fmaxf(v0.x, std::fmaxf(v1.x, v2.x)) < -e0 ||
      std::fminf(v0.x, std::fminf(v1.x, v2.x)) > e0)
    return 0;
  // ... [-e1, e1] and [min(v0.y,v1.y,v2.y), max(v0.y,v1.y,v2.y)] do not overlap
  if (std::fmaxf(v0.y, std::fmaxf(v1.y, v2.y)) < -e1 ||
      std::fminf(v0.y, std::fminf(v1.y, v2.y)) > e1)
    return 0;
  // ... [-e2, e2] and [min(v0.z,v1.z,v2.z), max(v0.z,v1.z,v2.z)] do not overlap
  if (std::fmaxf(v0.z, std::fmaxf(v1.z, v2.z)) < -e2 ||
      std::fminf(v0.z, std::fminf(v1.z, v2.z)) > e2)
    return 0;

...
bool Engine::Math::InRange(float valToCheck, float min, float max,
...
float Engine::Math::scaleAndBias(float valueToConvert, float oldRangeMin,
                                 float oldRangeMax, float newRangeMin,
                                 float newRangeMax) {
  /*
  ...
  + c
  */
  float ratioToMaintain =
      (newRangeMax - newRangeMin) / (oldRangeMax - oldRangeMin);

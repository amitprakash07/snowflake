...
class DebugMenu : public IMessageHandler, Object {
 public:
  ...
  std::map<int, Engine::SharedPointer<UI::UIElement>> GetDebugObjects() const;
  ...
  void CreateSlider(std::string iSliderName,
                    SharedPointer<MeshObject> associatedObject, float min,
                    float max);
  ...
  Graphics::RGBAColor GetVertexColor() const override {
    return Graphics::RGBAColor(1.0f);
  }
 ...
 private:
  ...
  std::map<int, Engine::SharedPointer<UI::UIElement>>
      mDebugMenuAssociatedObjectList;

...
Engine::SharedPointer<Engine::Camera> Engine::Camera::CreateCamera(
    std::string i_name, Math::Vector3 i_position,
    Math::Quaternion i_orientation, float i_fieldofView, float i_aspectRatio) {
  SharedPointer<Engine::Camera> tempCamera(
      new Engine::Camera(i_name, i_position, i_orientation, i_fieldofView,
                         i_aspectRatio),
      "Engine::Camera");
  Engine::utils::StringHash temp(
      Engine::EngineCore::getStringPool()->findString("UpdateObject"));
  Engine::utils::StringHash tempOne(
      Engine::EngineCore::getStringPool()->findString("ActionOnMouseEvent"));
...
float Engine::Camera::getFieldOfView() const { return fieldOfView; }

void Engine::Camera::setAspectRatio(float i_aspectRatio) {
  aspectRatio = i_aspectRatio;
}

float Engine::Camera::getAspectRatio() const { return aspectRatio; }

void Engine::Camera::activateCamera(bool i_activate) { active = i_activate; }

bool Engine::Camera::isActive() const { return active; }

Engine::Math::Transform Engine::Camera::getTransform() { return mTransform; }
...
void Engine::Camera::resetTransform() { mTransform = mInitialTransform; }

float Engine::Camera::getNearPlane() const { return nearPlane; }

float Engine::Camera::getFarPlane() const { return farPlane; }

std::string Engine::Camera::getCameraName() const { return mCameraName; }

void Engine::Camera::setFarPlane(float i_farPlane) { farPlane = i_farPlane; }

void Engine::Camera::setNearPlane(float i_nearPlane) {
  nearPlane = i_nearPlane;
}

void Engine::Camera::HandleMessage(Engine::utils::StringHash& i_message,
...
void Engine::Camera::setObjectController(
    SharedPointer<IObjectController> i_objectController) {
  if (!i_objectController.isNull()) mObjectController = i_objectController;
}

bool Engine::Camera::isCollidable() { return false; }

bool Engine::Camera::isPhysicsEnabled() { return false; }
...
bool Engine::Camera::castingShadowEnabled() { return false; }

bool Engine::Camera::receivingShadowEnabled() { return false; }

bool Engine::Camera::IsPlayer() { return isPlayer; }

void Engine::Camera::setPlayer(bool i_isPlayer) { isPlayer = i_isPlayer; }

Engine::ObjectType Engine::Camera::getObjectType() { return mObjectType; }

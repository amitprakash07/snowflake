...
Engine::SharedPointer<Engine::Time::FrameTime>
Engine::Time::FrameTime::getFrameTimeController() {
  ...
  return (SharedPointer<Engine::Time::FrameTime>(
      new FrameTime(tempFrequency, tempStartCounter),
      "Engine::Time::FrameTime"));
}

Engine::Time::FrameTime::FrameTime(LARGE_INTEGER *i_frequency,
...
void Engine::Time::FrameTime::updateDeltaTime() {
  ...
  deltaTime =
      static_cast<float>(Engine::Time::getTimeDiff(lastTick, currentTick) /
  ...
  if (pauseTimerTick != 0) {
    deltaTime -= static_cast<uint32_t>(
        (Engine::Time::getTimeDiff(pauseTimerTick, currentTick)) /
        frameTimeFrequency);
    pauseTimerTick = 0;
  }
...
float Engine::Time::FrameTime::getdeltaTime() { return deltaTime; }
...
float Engine::Time::FrameTime::getdeltaTimeDuringFrame() {
  ...
  float o_deltaTime =
      static_cast<float>(Engine::Time::getTimeDiff(lastTick, tempCurrentTick) /
                         static_cast<float>(frameTimeFrequency));
  delete tempTick;
  return o_deltaTime;
}

bool Engine::Time::FrameTime::getTimerStatus() { return isTimerPaused; }

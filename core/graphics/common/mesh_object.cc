...
Engine::SharedPointer<Engine::MeshObject> Engine::MeshObject::CreateMeshObject(
  ...
  SharedPointer<MeshObject> tempMeshObject(
      new MeshObject(i_meshFileName, i_materialName), "Engine::MeshObject");
  Engine::utils::StringHash temp(
      Engine::EngineCore::getStringPool()->findString("UpdateObject"));
  Engine::EngineCore::getMessagingSystem()->addMessageHandler(
  ...
  if (!i_meshFileName.empty() && !i_meshFileName.empty() &&
    ...
    if (!Engine::Graphics::Material::addMaterialToList(
...
Engine::MeshObject::MeshObject(std::string i_meshName,
...
void Engine::MeshObject::EnableDebugging(bool enable) { debugObject = enable; }

bool Engine::MeshObject::isDebugObject() const { return debugObject; }

void Engine::MeshObject::setObjectController(
    SharedPointer<IObjectController> i_objectController) {
  if (!i_objectController.isNull()) mObjectController = i_objectController;
}

Engine::SharedPointer<Engine::Graphics::Mesh> Engine::MeshObject::getMesh()
    const {
  return Engine::Graphics::Mesh::getMesh(mMeshName);
}

Engine::SharedPointer<Engine::Graphics::Effect> Engine::MeshObject::getEffect()
    const {
  return Engine::Graphics::Effect::getEffect(
      Engine::Graphics::Material::getMaterial(mMaterial.c_str())
          ->getEffectName());
}

Engine::SharedPointer<Engine::Graphics::Material>
Engine::MeshObject::getMaterial() const {
  return Engine::Graphics::Material::getMaterial(mMaterial.c_str());
}

bool Engine::MeshObject::isRenderable() const { return renderable; }

void Engine::MeshObject::setTransform(Engine::Math::Vector3 i_positionOffset,
...
void Engine::MeshObject::resetTransform() { mTransform = mInitialTransform; }

void Engine::MeshObject::HandleMessage(Engine::utils::StringHash& i_message,
                                       SharedPointer<RTTI> i_MessageSender,
                                       void* i_pMessageData) {
  if (!i_MessageSender.isNull()) {
    if (!i_MessageSender.isNull() &&
        Engine::utils::StringHash("UpdateObject") == i_message &&
        !mObjectController.isNull())
      mObjectController->updateObject(
          *this, *reinterpret_cast<Engine::typedefs::ActionWithKeyBound*>(
                     i_pMessageData));
  }
}
...
void Engine::MeshObject::draw(bool drawDebugObject) {
  if (renderable && (drawDebugObject && debugObject || !debugObject)) {
    ...
    if (!tempCamera.isNull()) {
      ...
      if (debugObject) Engine::Graphics::GraphicsSystem::EnableWireFrame(true);
      ...
      if (!scaleMatrix.isNull()) {
        ...
        scaleMatrixValues.matrixValue.matrix =
            Engine::Math::Matrix4x4::CreateScaleMatrix(scaleFactor);
      ...
      if (!normalMatrix.isNull()) {
        ...
        normalMatrixValues.matrixValue.matrix =
            Engine::Math::Matrix4x4::CreateNormalMatrix(Math::Matrix4x4(
                mTransform.getOrientation(), mTransform.getPosition()));
      ...
      Engine::Graphics::UniformBlock::setAllUniformBlockForTheEffectInShader(
          effectFile);

      getMaterial()->setMaterialUniformParameters();
      getMaterial()->setTextureUniform();
      getMesh()->drawMesh();
      if (debugObject) Engine::Graphics::GraphicsSystem::EnableWireFrame(false);
    } else {
      std::stringstream errormessage;
...
bool Engine::MeshObject::IsPlayer() { return isPlayer; }

void Engine::MeshObject::setPlayer(bool i_isPlayer) { isPlayer = i_isPlayer; }

Engine::ObjectType Engine::MeshObject::getObjectType() { return mObjectType; }
...
void Engine::MeshObject::SetVertexColor(float iR, float iG, float iB,
                                        float iA) {
...
void Engine::MeshObject::EnableSSAO(bool iRequest) { isSSAO = iRequest; }

bool Engine::MeshObject::isSSAOEnabled() { return isSSAO; }

...
Engine::Math::Vector3 Engine::Physics::Octree::GetMax() const { return max; }

Engine::Math::Vector3 Engine::Physics::Octree::GetMin() const { return min; }

uint32_t Engine::Physics::Octree::GetNodeCount() const { return mCountOfNode; }

Engine::Physics::OctreeNode* Engine::Physics::Octree::GetNodes() const {
  return mNodes;
}

uint32_t Engine::Physics::Octree::Level() const { return mLevel; }

uint32_t Engine::Physics::Octree::GetCountOfNodesInLevel(
    uint32_t levelIndex) const {
  return static_cast<uint32_t>(std::pow(8.0f, levelIndex));
}

Engine::Physics::OctreeNode* Engine::Physics::Octree::GetNodesInLevel(
    uint32_t levelIndex) const {
  uint32_t baseOfNodesInLevel =
      static_cast<uint32_t>((std::pow(8.0f, levelIndex) - 1) / (8 - 1));
  uint32_t countOfNodesInLevel =
      static_cast<uint32_t>(std::pow(8.0f, levelIndex));
  return &mNodes[baseOfNodesInLevel];
}

void Engine::Physics::Octree::InitFromRange(uint32_t level,
  ...
  for (uint32_t levelIndex = 0; levelIndex < mLevel - 1; ++levelIndex) {
    ...
    for (uint32_t nodeIndex = 0; nodeIndex < countOfNodesInLevel; ++nodeIndex) {
      ...
      pChildren[0].pos =
          centerPos + Math::Vector3(-extentInNextLevel.x, -extentInNextLevel.y,
                                    -extentInNextLevel.z);
      pChildren[1].pos =
          centerPos + Math::Vector3(-extentInNextLevel.x, -extentInNextLevel.y,
                                    +extentInNextLevel.z);
...
void Engine::Physics::Octree::InitFromFile(const char* pFile) {
  ...
  {
    uint32_t offset = 0;
    memcpy(reinterpret_cast<uint8_t*>(pBuffer) + offset,
           reinterpret_cast<uint8_t*>(&mLevel), sizeof(uint32_t));
    offset += sizeof(uint32_t);
    memcpy(reinterpret_cast<uint8_t*>(pBuffer) + offset,
           reinterpret_cast<uint8_t*>(&mCountOfNode), sizeof(uint32_t));
    offset += sizeof(uint32_t);
    memcpy(reinterpret_cast<uint8_t*>(pBuffer) + offset,
           reinterpret_cast<uint8_t*>(&min), sizeof(Engine::Math::Vector3));
    offset += sizeof(Engine::Math::Vector3);
    memcpy(reinterpret_cast<uint8_t*>(pBuffer) + offset,
           reinterpret_cast<uint8_t*>(&max), sizeof(Engine::Math::Vector3));
    offset += sizeof(Engine::Math::Vector3);
    ...
    for (uint32_t leafIndex = 0; leafIndex < countOfLeaves; ++leafIndex) {
      ...
      memcpy(reinterpret_cast<uint8_t*>(pBuffer + offset),
             reinterpret_cast<uint8_t*>(&triangleCountInLeaf),
             sizeof(uint32_t));
      offset += sizeof(uint32_t);
      if (triangleCountInLeaf > 0) {
        for (uint32_t trianlgeIndex = 0; trianlgeIndex < triangleCountInLeaf;
             ++trianlgeIndex) {
          TriangleIndex tempTraiangle;
          memcpy(reinterpret_cast<uint8_t*>(pBuffer + offset),
                 reinterpret_cast<uint8_t*>(&tempTraiangle),
                 sizeof(TriangleIndex));

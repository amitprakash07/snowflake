...
struct OctreeNode {
  ...
  Engine::Math::Vector3 GetMin() const { return pos - extent; }
  Engine::Math::Vector3 GetMax() const { return pos + extent; }
};
...
class Octree {
 public:
  Octree();
  ~Octree();
  void InitFromRange(uint32_t level, Engine::Math::Vector3 min,

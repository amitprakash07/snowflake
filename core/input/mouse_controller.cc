...
Engine::SharedPointer<Engine::MouseController>
    Engine::MouseController::mMouseInputController;

std::string Engine::MouseController::getTypeInfo() const { return mTypeName; }

bool Engine::MouseController::isBothSameType(SharedPointer<RTTI> i_first,
...
Engine::MouseController::getMouseController() {
  if (mMouseInputController.isNull()) {
    SharedPointer<MouseController> tempInputController(
        Engine::SharedPointer<MouseController>(
            new MouseController(),
            Engine::EngineCore::getStringPool()->findString(
                "Engine::InputController")));
    mMouseInputController = tempInputController;
    Engine::utils::StringHash temp = Engine::utils::StringHash(
        Engine::EngineCore::getStringPool()->findString("MouseEvent"));
...
void Engine::MouseController::HandleMessage(Engine::utils::StringHash&,
                                            SharedPointer<RTTI>,
                                            void* i_pMessageData) {
...
Engine::MouseEventAndPosition Engine::MouseController::getCurrentMouseState()
    const {
...
void Engine::MouseController::setMousePosition(int i_x, int i_y,
                                               POINTS i_point) {

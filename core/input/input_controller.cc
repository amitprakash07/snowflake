...
Engine::SharedPointer<Engine::InputController>
    Engine::InputController::mInputController;

std::string Engine::InputController::getTypeInfo() const { return mTypeName; }

bool Engine::InputController::isBothSameType(SharedPointer<RTTI> i_first,
                                             std::string i_second) const {
  if (i_first->getTypeInfo() == i_second) return true;
  return false;
}

void Engine::InputController::HandleMessage(
    Engine::utils::StringHash& i_message, SharedPointer<RTTI> i_MessageSender,
    void* i_pMessageData) {
...
Engine::InputController::getInputController() {
  if (mInputController.isNull()) {
    SharedPointer<InputController> tempInputController(
        Engine::SharedPointer<InputController>(
            new InputController(),
